...
# 촬영 이미지가 작아 특징점 포착 못하는 문제 발생, 포착 가능할때까지 프레임 건너 뛰기 적용했지만 특징점 포착 실패

def video_stitching():
    global image_list_route
    image_list_route = []

    # 비디오 파일 열기
    cap = cv.VideoCapture(r'C:\Users\jangj\PycharmProjects\pythonProject1\image stitching\route video.mp4')
    ...
    while True:
        ...
        # matches, H, status가 None이 아닌 경우에만 drawMatches 함수 호출
        if matches is not None and H is not None and status is not None:
            # 매칭 결과 이미지 생성
            img_matching_result = drawMatches(prev_frame, current_frame, keyPoints1, keyPoints2, matches, status)

            # matches가 부족한 경우 프레임 건너뛰기
            if len(matches) < 4:
            ...
            try:
                ...
                # 이미지 파일의 이름 기록
                image_name = f'preprocessing(route){cap.get(cv.CAP_PROP_POS_FRAMES)}.png'
                image_list_route.append(image_name)
    ...
    # 이미지 파일 이름 기록
    with open('image_list(route).txt', 'w') as file:
        for item in image_list_route:
            file.write("%s\n" % item)
